...
Public Class FrmPhysicalStockAdjustmentEntry
    ...
    Public Sub New(ByVal StrUPVar As String, ByVal DTUP As DataTable, ByVal NCatStr As String, ByVal ForNCatStr As String, ByVal ItemTypeStr As String)
        ...
        EntryNCat = NCatStr
        mForNCat = ForNCatStr
        mItemTypeStr = Replace(ItemTypeStr, ",", "','")
    End Sub
...
#End Region
    ...
    Private Sub FrmSaleOrder_BaseFunction_IniGrid() Handles Me.BaseFunction_IniGrid
        ...
        With AgCL
            ...
            .AddAgTextColumn(Dgl1, Col1Item, 120, 0, Col1Item, True, True, False)
            .AddAgTextColumn(Dgl1, Col1Process, 70, 0, Col1Process, True, True, False)
            ...
            .AddAgNumberColumn(Dgl1, Col1TotalMeasure, 90, 8, 4, False, Col1TotalMeasure, True, True, True)
            .AddAgTextColumn(Dgl1, Col1MeasureUnit, 65, 0, Col1MeasureUnit, True, True, False)
            .AddAgNumberColumn(Dgl1, Col1CurrentStockMeasure, 90, 8, 4, False, Col1CurrentStockMeasure, True, True, True)
            .AddAgNumberColumn(Dgl1, Col1DifferenceQty, 70, 8, 2, False, Col1DifferenceQty, True, True, True)
            .AddAgNumberColumn(Dgl1, Col1DifferenceMeasure, 90, 8, 2, False, Col1DifferenceMeasure, True, True, True)
        End With
            ...
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    ...
                    mQry = "INSERT INTO StockHeadDetail(DocId, Sr, Item, Process, " & _
                            ...
                            " DifferenceQty, DifferenceMeasure) " & _
    ...
    Private Sub ChkAssumeStock0IfNotFound_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ChkAssumeStock0IfNotFound.Click
        ...
        Try
            ...
            If ChkAssumeStock0IfNotFound.Checked Then
                For I = 0 To Dgl1.Rows.Count - 1
                    If Dgl1.Item(Col1Item, I).Value <> "" Then
                        If PhysicalStockItemWiseQry = "" Then
                            PhysicalStockItemWiseQry = " Select " & AgL.Chk_Text(Dgl1.Item(Col1Item, I).Tag) & " As Item, " & _
                ...
                ComputerStockQry = " SELECT L.Item, IsNull(Sum(L.Qty_Rec),0) - IsNull(Sum(L.Qty_Iss),0) AS ComputerStock, 0 As PhysicalStock " & _
                           " FROM Stock L " & _
                           " LEFT JOIN Item I ON L.Item = I.Code " & _
                           " WHERE L.V_Date <= '" & TxtV_Date.Text & "'   " & _
                           " And I.ItemType In ('" & mItemTypeStr & "') " & _
                ...
                'ComputerStockQry = " SELECT L.Item, IsNull(Sum(L.Qty_Rec),0) - IsNull(Sum(L.Qty_Iss),0) AS ComputerStock, 0 As PhysicalStock  " & _
                '           " FROM ( " & _
                '           " SELECT S1.Item, S1.Qty_Rec, S1.Qty_Iss FROM Stock S1 " & _
                '           " LEFT JOIN Item I ON S1.Item = I.Code " & _
                '           " WHERE S1.V_Date <= '" & TxtV_Date.Text & "'   " & _
                '           " And I.ItemType = '" & ClsMain.ItemType.CarpetSKU & "' " & _
                '           " And S1.Godown = '" & TxtGodown.Tag & "'" & _
                '           " And S1.Site_Code = '" & AgL.PubSiteCode & "'" & _
                '           " UNION ALL " & _
                '           " SELECT S2.Item, S2.Qty_Rec, S2.Qty_Iss FROM StockProcess S2 " & _
                '           " LEFT JOIN Item I ON S2.Item = I.Code " & _
                '           " WHERE S2.V_Date <= '" & TxtV_Date.Text & "'   " & _
                '           " And I.ItemType = '" & ClsMain.ItemType.CarpetSKU & "' " & _
                '           " And S2.Godown = '" & TxtGodown.Tag & "'" & _
                '           " And S2.Site_Code = '" & AgL.PubSiteCode & "'" & _
                '           " ) AS L " & _
                '           " Group By L.Item "

                If PhysicalStockItemWiseQry <> "" Then
                    mQry = " Select VMain.Item, Max(I.Description) As ItemDesc, " & _
